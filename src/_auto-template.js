...
class AutoTemplate {
    ...
    clear(opt = 2) {
        ...
        for (let i = 0; i < this._buildFile['cover'].length; i++) {
            filePath = this.dir + path.sep + this._buildFile['cover'][i].tar;
        ...
        for (let i = 0; i < this._buildFile['publish'].length; i++) {
            filePath = this.dir + path.sep + this._buildFile['publish'][i].tar;
        ...
        // 빈 객체 삭제
        // this._buildFile['cover'].forEach(val => {
        //     if (!fs.existsSync(val.ori) || fs.existsSync(val.tar) ) 
        // });
    ...
    _addBuildFile(savePath, type) {
        ...
        // } else if (type === 'publish' && !this._buildFile['publish'].find(val => val.tar === savePath.tar)) {
            ...
            // focus 설정
            // if (this._buildFile['focus'][localPath.tar])
            this._buildFile['focus'][localPath.tar] = localPath.ori;
